...
public class NodeInfo {
	...
	public RuntimeStatistics getRuntimeStatistics() {
		return new RuntimeStatistics(
				lastReceived.get_routing_data_ack_failed(), lastReceived
						.get_routing_data_ack_received(), lastReceived
						.get_routing_data_beacons(), lastReceived
						.get_routing_data_dropped_duplicates(), duplicates,
				lastParent(), lastReceived.get_routing_data_seq_no() + 1, id,
				parentOverflow, parents.cardinality(), lastReceived
						.get_routing_data_tx_queue_full(), lastReceived
						.get_temperature(), lastReceived.get_humidity(),
	...
	public NodeStatistics getStatistics() {
		...
		offlineStats = new NodeStatistics(lastReceived
				.get_routing_data_ack_failed(), lastReceived
				.get_routing_data_ack_received(), lastReceived
				.get_routing_data_beacons(), lastReceived
				.get_routing_data_dropped_duplicates(), duplicates,
				lastParent(), lostPackets, lastReceived
						.get_routing_data_seq_no() + 1, id, parentOverflow,
				parentChanges, parents.cardinality(), lastReceived
						.get_routing_data_tx_queue_full());
		return offlineStats;
	}
